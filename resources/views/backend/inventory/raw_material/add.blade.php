...
@section('content')
	<section class="app-user-list">
		...
		<div class="card" style="padding: 21px;">
			...
			<div class="card-body">
				<form id="raw-material-form" action="{{ route('raw_materials.save') }}" method="post">
					@csrf
					<div class="row mb-1">
						<label for="shipment-date" class="col-sm-2 col-form-label">Shipment Date</label>
						<div class="col-sm-10">
		                    <input type="text" class="form-control" name="shipment_date" value="" />
						</div>
					</div>
					...
					<div class="row mb-1">
						...
						<div class="col-sm-10">
							<div class="form-check form-check-inline">
								<input type="radio" id="status-active" name="status" value="1" class="form-check-input" checked/>
								<label class="form-check-label" for="active">Active</label>
							</div>
							<div class="form-check form-check-inline">
								<input type="radio" id="status-deactive" name="status" value="0" class="form-check-input"/>
						...
						<div class="col-sm-10">
	                        <select class="form-select material-type" name="material_type" id="material-type" data-live-search="true">
	                            ...
	                            @foreach ($raw_material_type_list as $raw_material_type)
	                            	<option value="{{ $raw_material_type->id }}">{{ $raw_material_type->name }}</option>
	                            @endforeach
	                        </select>
						</div>
					</div>

					<span class="col-lg-12 attribute-div"></span>

					<div class="row mb-1">
						<div class="col-sm-12 text-end">
							<button type="submit" class="btn btn-outline-primary"><i data-feather='save'></i> Save</button>
						</div>
					</div>
				</form>
			</div>
